...
class TradingBot:
    ...
    def start(self):
        ...
        if settings.ENABLE_BREAKOUT: schedule.every(8).minutes.do(self._scan_breakout)
        if settings.ENABLE_SCALPING: schedule.every(45).seconds.do(self._scan_scalping)
        schedule.every(4).minutes.do(self._scan_emerging)
        schedule.every(30).seconds.do(self._monitor_positions)
        schedule.every(1).hours.do(self._health_check)
        schedule.every(10).minutes.do(lambda: self._sentiment.get_sentiment(force=True))
        schedule.every(20).minutes.do(lambda: self._emerging.scan(force=True))
        schedule.every(5).minutes.do(self._check_regime)
        schedule.every().day.at("00:05").do(self._daily_report)
        if DASHBOARD_ENABLED: schedule.every(1).minutes.do(lambda: write_state(self))
        self._scan_swing(); self._scan_breakout()
